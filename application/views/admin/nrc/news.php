			...
			<div class="row">
				<div class="col-lg-7 col-md-6 col-sm-12">
					...
					<ul class="breadcrumb">
						...
						<li class="breadcrumb-item"><a href="javascript:void(0);">NRC News </a></li>
						<li class="breadcrumb-item active"> <?=isset($details)?'Edit':'Add'?> NRC News</li>
					</ul>
		...
		<div class="container-fluid">
			...
			<div class="row clearfix">
				<div class="col-lg-12 col-md-12 col-sm-12">
					...
					<div class="card">
						<div class="header">
							<h2><strong><?=isset($details)?'Edit':'Add'?> NRC News </strong> </h2>
						</div>
						<div class="body">
							<form id="frmNrcNews" method="" action="" enctype="multipart/form-data">
								<label> Select Category</label>
								<div class="form-group">
									<select class="form-control show-tick ms select2" data-placeholder="Select" name="nrc_categorie_id" id="nrc_categorie_id" required>
                                        <option value=""> -- Select Category -- </option>
                                        <?php
                                            if($nrc_categories){
                                                foreach($nrc_categories as $value){
                                                    ?>
                                                        <option value="<?=$value->nrc_categorie_id?>"
                                                        <?= (isset($details) && $details->nrc_categorie_id == $value->nrc_categorie_id)?'selected':''?>
                                                        ><?=$value->name?></option>
                                                    <?php
                                                }
                                            }
                                ...
                                <input type="hidden" name="nrc_news_id" value="<?= isset($details) ? $details->nrc_news_id : ''?>">
...
<script>
   $(document).ready(function () {
    ...
    $('#frmNrcNews').submit(function(e){
        ...
        $.ajax({
            type: "POST",
            url: base_url + "nrc/News/newsSave",
            data: fd,
            processData: false,
            contentType: false,
            beforeSend: function() {
            ...
            success: function(response) {
              console.log(response);           
                if(response.status.error_code == 0){
                    swalAlertThenRedirect(response.status.message, 'success', base_url+"nrc/news-list");
                }else{
