		...
		<div class="block-header">
			<div class="row">
				<div class="col-lg-7 col-md-6 col-sm-12">
					<h2><?=isset($details)?'Edit':'Add'?> Legal News </h2>
					<ul class="breadcrumb">
						<li class="breadcrumb-item"><a href="<?=base_url('admin')?>"><i class="zmdi zmdi-home"></i> Genocide </a>
						</li>
						<li class="breadcrumb-item"><a href="javascript:void(0);">Legal News </a></li>
						<li class="breadcrumb-item active"> <?=isset($details)?'Edit':'Add'?> Legal News</li>
					</ul>
		...
		<div class="container-fluid">
			...
			<div class="row clearfix">
				<div class="col-lg-12 col-md-12 col-sm-12">
					...
					<div class="card">
						<div class="header">
							<h2><strong><?=isset($details)?'Edit':'Add'?> Legal News </strong> </h2>
						</div>
						<div class="body">
							<form id="frmLegalNews" method="" action="" enctype="multipart/form-data">
								<label> Select Category</label>
								<div class="form-group">
									<select class="form-control show-tick ms select2" data-placeholder="Select" name="legal_categorie_id" id="legal_categorie_id" required>
                                        <option value=""> -- Select Category -- </option>
                                        <?php
                                            if($legal_categories){
                                                foreach($legal_categories as $value){
                                                    ?>
                                                        <option value="<?=$value->legal_categorie_id?>"
                                                        <?= (isset($details) && $details->legal_categorie_id == $value->legal_categorie_id)?'selected':''?>
                                                        ><?=$value->name?></option>
                                                    <?php
                                                }
                                            }
                                ...
                                <input type="hidden" name="legal_news_id" value="<?= isset($details) ? $details->legal_news_id : ''?>">
								<a href="<?= base_url('admin/legal/news-list') ?>" class="btn btn-raised btn-info btn-round waves-effect">Back</a>
...
<script>
   $(document).ready(function () {
    ...
    $('#frmLegalNews').submit(function(e){
        e.preventDefault();
        if($('#legal_categorie_id').val() == ""){
            ...
            success: function(response) {
              console.log(response);           
                if(response.status.error_code == 0){
                    swalAlertThenRedirect(response.status.message, 'success', base_url+"legal/news-list");
                }else{
