...
class Category extends MY_Controller {
	...
	public function index()
	{
		...
		$this->data['page'] = 'admin/gallery/category';
	...
	public function categorySave()
	{
		...
		if (!empty($postData)) {
			...
			if(empty($postData['id'])){
				$isData = $this->common_model->select_row('gallery_categories', ['LOWER(name)'=> strtolower($categoryName)], '*');
			...
			if(empty($postData['id'])){//
				if($this->common_model->add('gallery_categories', $this->data)){
				...
				if($this->common_model->update('gallery_categories', $this->data,['gallery_category_id '=> $postData['id']])){
	...
	public function get()
	{
		...
		if (!empty($postData)) {
			...
			$select = 'categories.*';
			$this->obj = $this->common_model->select('gallery_categories categories', $where, $select, 'categories.gallery_category_id ', 'DESC');
			//echo $this->db->last_query(); die;

			if($postData['source'] === 'WEB'){
				$this->data['gallery_categories'] = $this->obj;
