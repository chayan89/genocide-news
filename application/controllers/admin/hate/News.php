...
class News extends MY_Controller {
	...
	{
		$this->checkAdminAuth();
		$this->data['page'] = 'admin/hate/news-list';
	...
	{
		$this->checkAdminAuth();
		$this->data['hate_categories'] = $this->common_model->select('hate_categories', ['status'=> 1], '*', 'name', 'ASC');
		$this->data['states'] = $this->common_model->select('states', ['status'=> 1], '*', 'state_name', 'ASC');
		$this->data['page'] = 'admin/hate/news';
		$this->load_view($this->data);
	}
	/**
	 * Load product add view
	...
	public function edit($id = null)
	{
		$this->checkAdminAuth();
		if($id != null){
			$this->data['details'] = $this->common_model->select_row('hate_news', ['hate_news_id'=> $id], 'hate_news.*');
			$this->data['details']->images = $this->common_model->select('hate_news_images', ['hate_news_id'=> $id], 'hate_news_images.*');
		}
		$this->data['hate_categories'] = $this->common_model->select('hate_categories', ['status'=> 1], '*', 'name', 'ASC');
		$this->data['states'] = $this->common_model->select('states', ['status'=> 1], '*', 'state_name', 'ASC');
		$this->data['page'] = 'admin/hate/news';
	...
	public function hateSave()
	{ 
		$postData = $this->input->post();
		if (!empty($postData)) {
			// echo '<pre>';
			// print_r($postData);
			if($postData['hate_categorie_id'] ==""){
			...
			if(empty($postData['hate_news_id'])){
			    $this->data['created_at'] = date('Y-m-d H:i:s');
				if($hate_news_id = $this->common_model->add('hate_news', $this->data)){
					/**
					 * Add multiple images 
					*/
					$this->data = [];
					for($i=0; $i<count($_FILES['image']['name']); $i++)
					{
						...
						$ext = pathinfo($filename, PATHINFO_EXTENSION);						
						if (in_array($ext, $allowed)) {
							$image_file = $i.'_'.time()."_hate." . $ext;
							$imgPath = getcwd()."/uploads/hate/".$image_file;
							if(move_uploaded_file($_FILES['image']['tmp_name'][$i], $imgPath)){
								$this->data[] = array(
										'hate_news_id' => $hate_news_id,
										'news_image' => $image_file
									);
							}
						}
						if(!empty($this->data)){
							$this->common_model->batch_insert('hate_news_images' ,$this->data);
						}
					}
				...
				if($this->common_model->update('hate_news', $this->data,['hate_news_id'=> $postData['hate_news_id']])){
					/**
					 * Add multiple images 
					*/
					$this->data = [];
					for($i=0; $i<count($_FILES['image']['name']); $i++)
					{
						...
						if(!empty($this->data)){
							//remove old images
							$this->db->where(['hate_news_id'=> $postData['id']]);
							$this->db->delete('hate_news_images');
							//insert new images
							$this->common_model->batch_insert('hate_news_images' ,$this->data);
						}
					}
	...
	public function get()
	{
		...
		if (!empty($postData)) {
			...
			// }
			$this->join[] = ['table' => 'hate_categories cat', 'on' => 'cat.hate_categorie_id = news.hate_categorie_id', 'type' => 'left'];
			$this->join[] = ['table' => 'states', 'on' => 'states.state_id = news.state_id', 'type' => 'left'];
			$this->obj = $this->common_model->select('hate_news news', $where, $select, 'news.hate_news_id', 'DESC', $this->join);
			//echo $this->db->last_query(); die;

			if($postData['source'] === 'WEB'){
				$this->data['hate_news'] = $this->obj;
