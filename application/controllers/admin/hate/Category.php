...
class Category extends MY_Controller {
	...
	public function index()
	{
		...
		$this->data['page'] = 'admin/hate/category';
	...
	public function categorySave()
	{
		...
		if (!empty($postData)) {
			...
			if(empty($postData['id'])){
				$isData = $this->common_model->select_row('hate_categories', ['LOWER(name)'=> strtolower($categoryName)], '*');
			
				if(!empty($isData)){
					// $this->session->set_flashdata('error', 'Category Name already exists');
					// redirect('admin/hate/category', 'refresh');
			...
			if(empty($postData['id'])){//
				...
				if($this->common_model->add('hate_categories', $this->data)){
				...
				if($this->common_model->update('hate_categories', $this->data,['hate_categorie_id '=> $postData['id']])){
	...
	public function get()
	{
		...
		if (!empty($postData)) {
			...
			$select = 'categories.*';
			$this->obj = $this->common_model->select('hate_categories categories', $where, $select, 'categories.hate_categorie_id ', 'DESC');
			//echo $this->db->last_query(); die;

			if($postData['source'] === 'WEB'){
				$this->data['hate_categories'] = $this->obj;
