...
class News extends MY_Controller {
	...
	{
		$this->checkAdminAuth();
		$this->data['page'] = 'admin/genocide/news-list';
	...
	{
		$this->checkAdminAuth();
		$this->data['genocide_categories'] = $this->common_model->select('genocide_categories', ['status'=> 1], '*', 'name', 'ASC');
		$this->data['states'] = $this->common_model->select('states', ['status'=> 1], '*', 'state_name', 'ASC');
		$this->data['page'] = 'admin/genocide/news';
		$this->load_view($this->data);
	}
	/**
	 * Load product add view
	...
	public function edit($id = null)
	{
		$this->checkAdminAuth();
		if($id != null){
			$this->data['details'] = $this->common_model->select_row('genocide_news', ['genocide_news_id'=> $id], 'genocide_news.*');
			$this->data['details']->images = $this->common_model->select('genocide_news_images', ['genocide_news_id'=> $id], 'genocide_news_images.*');
		}
		$this->data['genocide_categories'] = $this->common_model->select('genocide_categories', ['status'=> 1], '*', 'name', 'ASC');
		$this->data['states'] = $this->common_model->select('states', ['status'=> 1], '*', 'state_name', 'ASC');
		$this->data['page'] = 'admin/genocide/news';
	...
	public function genocideSave()
	{ 
		$postData = $this->input->post();
		if (!empty($postData)) {
			// echo '<pre>';
			// print_r($postData);
			if($postData['genocide_categorie_id'] ==""){
			...
			if(empty($postData['genocide_news_id'])){
				if($genocide_news_id = $this->common_model->add('genocide_news', $this->data)){
					/**
					 * Add multiple images 
					*/
					$this->data = [];
					for($i=0; $i<count($_FILES['image']['name']); $i++)
					{
						...
						$ext = pathinfo($filename, PATHINFO_EXTENSION);						
						if (in_array($ext, $allowed)) {
							$image_file = $i.'_'.time()."_genocide." . $ext;
							$imgPath = getcwd()."/uploads/genocide/".$image_file;
							if(move_uploaded_file($_FILES['image']['tmp_name'][$i], $imgPath)){
								$this->data[] = array(
										'genocide_news_id' => $genocide_news_id,
										'news_image' => $image_file
									);
							}
						}
						if(!empty($this->data)){
							$this->common_model->batch_insert('genocide_news_images' ,$this->data);
						}
					}
				...
				if($this->common_model->update('genocide_news', $this->data,['genocide_news_id'=> $postData['genocide_news_id']])){
					/**
					 * Add multiple images 
					*/
					$this->data = [];
					for($i=0; $i<count($_FILES['image']['name']); $i++)
					{
						...
						if(!empty($this->data)){
							//remove old images
							$this->db->where(['genocide_news_id'=> $postData['id']]);
							$this->db->delete('genocide_news_images');
							//insert new images
							$this->common_model->batch_insert('genocide_news_images' ,$this->data);
						}
					}
	...
	public function get()
	{
		...
		if (!empty($postData)) {
			...
			// }
			$this->join[] = ['table' => 'genocide_categories cat', 'on' => 'cat.genocide_categorie_id = news.genocide_categorie_id', 'type' => 'left'];
			$this->join[] = ['table' => 'states', 'on' => 'states.state_id = news.state_id', 'type' => 'left'];
			$this->obj = $this->common_model->select('genocide_news news', $where, $select, 'news.genocide_news_id', 'DESC', $this->join);
			//echo $this->db->last_query(); die;

			if($postData['source'] === 'WEB'){
				$this->data['genocide_news'] = $this->obj;
