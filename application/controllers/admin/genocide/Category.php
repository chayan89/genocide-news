...
class Category extends MY_Controller {
	...
	public function index()
	{
		...
		$this->data['page'] = 'admin/genocide/category';
	...
	public function categorySave()
	{
		...
		if (!empty($postData)) {
			...
			if(empty($postData['id'])){
				$isData = $this->common_model->select_row('genocide_categories', ['LOWER(name)'=> strtolower($categoryName)], '*');
			
				if(!empty($isData)){
					// $this->session->set_flashdata('error', 'Category Name already exists');
					// redirect('admin/genocide/category', 'refresh');
			...
			if(empty($postData['id'])){//
				if($this->common_model->add('genocide_categories', $this->data)){
				...
				if($this->common_model->update('genocide_categories', $this->data,['genocide_categorie_id '=> $postData['id']])){
	...
	public function get()
	{
		...
		if (!empty($postData)) {
			...
			$select = 'categories.*';
			$this->obj = $this->common_model->select('genocide_categories categories', $where, $select, 'categories.genocide_categorie_id ', 'DESC');
			//echo $this->db->last_query(); die;

			if($postData['source'] === 'WEB'){
				$this->data['genocide_categories'] = $this->obj;
