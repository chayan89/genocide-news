...
class Category extends MY_Controller {
	...
	public function index()
	{
		...
		$this->data['page'] = 'admin/legal/category';
	...
	public function categorySave()
	{
		...
		if (!empty($postData)) {
			...
			if(empty($postData['id'])){
				$isData = $this->common_model->select_row('legal_categories', ['LOWER(name)'=> strtolower($categoryName)], '*');
			
				if(!empty($isData)){
					// $this->session->set_flashdata('error', 'Category Name already exists');
					// redirect('admin/legal/category', 'refresh');
			...
			if(empty($postData['id'])){//
				...
				if($this->common_model->add('legal_categories', $this->data)){
				...
				if($this->common_model->update('legal_categories', $this->data,['legal_categorie_id '=> $postData['id']])){
	...
	public function get()
	{
		...
		if (!empty($postData)) {
			...
			$select = 'categories.*';
			$this->obj = $this->common_model->select('legal_categories categories', $where, $select, 'categories.legal_categorie_id ', 'DESC');
// 			echo $this->db->last_query();
// 			print_r($this->obj);
// 			die;

			if($postData['source'] === 'WEB'){
				$this->data['legal_categories'] = $this->obj;
