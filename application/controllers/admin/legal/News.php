...
class News extends MY_Controller {
	...
	{
		$this->checkAdminAuth();
		$this->data['page'] = 'admin/legal/news-list';
	...
	{
		$this->checkAdminAuth();
		$this->data['legal_categories'] = $this->common_model->select('legal_categories', ['status'=> 1], '*', 'name', 'ASC');
		$this->data['states'] = $this->common_model->select('states', ['status'=> 1], '*', 'state_name', 'ASC');
		$this->data['page'] = 'admin/legal/news';
		$this->load_view($this->data);
	}
	/**
	 * Load product add view
	...
	public function edit($id = null)
	{
		$this->checkAdminAuth();
		if($id != null){
			$this->data['details'] = $this->common_model->select_row('legal_news', ['legal_news_id'=> $id], 'legal_news.*');
			$this->data['details']->images = $this->common_model->select('legal_news_images', ['legal_news_id'=> $id], 'legal_news_images.*');
		}
		$this->data['legal_categories'] = $this->common_model->select('legal_categories', ['status'=> 1], '*', 'name', 'ASC');
		$this->data['states'] = $this->common_model->select('states', ['status'=> 1], '*', 'state_name', 'ASC');
		$this->data['page'] = 'admin/legal/news';
	...
	{ 
		...
		if (!empty($postData)) {
			// echo '<pre>';
			// print_r($postData);
			if($postData['legal_categorie_id'] ==""){
			...
			if(empty($postData['legal_news_id'])){
				$this->data['created_at'] = date('Y-m-d H:i:s');
				if($legal_news_id = $this->common_model->add('legal_news', $this->data)){
					/**
					 * Add multiple images 
					*/
					$this->data = [];
					for($i=0; $i<count($_FILES['image']['name']); $i++)
					{
						...
						$ext = pathinfo($filename, PATHINFO_EXTENSION);						
						if (in_array($ext, $allowed)) {
							$image_file = $i.'_'.time()."_legal." . $ext;
							$imgPath = getcwd()."/uploads/legal/".$image_file;
							if(move_uploaded_file($_FILES['image']['tmp_name'][$i], $imgPath)){
								$this->data[] = array(
										'legal_news_id' => $legal_news_id,
										'news_image' => $image_file
									);
							}
						}
						if(!empty($this->data)){
							$this->common_model->batch_insert('legal_news_images' ,$this->data);
						}
					}
				...
				if($this->common_model->update('legal_news', $this->data,['legal_news_id'=> $postData['legal_news_id']])){
					/**
					 * Add multiple images 
					*/
					$this->data = [];
					for($i=0; $i<count($_FILES['image']['name']); $i++)
					{
						...
						if(!empty($this->data)){
							//remove old images
							$this->db->where(['legal_news_id'=> $postData['id']]);
							$this->db->delete('legal_news_images');
							//insert new images
							$this->common_model->batch_insert('legal_news_images' ,$this->data);
						}
					}
	...
	public function get()
	{
		...
		if (!empty($postData)) {
			...
			// }
			$this->join[] = ['table' => 'legal_categories cat', 'on' => 'cat.legal_categorie_id = news.legal_categorie_id', 'type' => 'left'];
			$this->join[] = ['table' => 'states', 'on' => 'states.state_id = news.state_id', 'type' => 'left'];
			$this->obj = $this->common_model->select('legal_news news', $where, $select, 'news.legal_news_id', 'DESC', $this->join);
			//echo $this->db->last_query(); die;

			if($postData['source'] === 'WEB'){
				$this->data['legal_news'] = $this->obj;
