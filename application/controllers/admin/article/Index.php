...
class Index extends MY_Controller {
	...
	public function index()
	{
		...
		$this->data['page'] = 'admin/article/index';
	...
	{ 
		...
		if (!empty($postData)) {
			...
			if(empty($postData['article_id'])){
			    $this->data['created_at'] = date('Y-m-d');
				if($article_id = $this->common_model->add('articles', $this->data)){
				...
				// 	}
					...
					$this->response = array('status' => array('error_code' => 0, 'message' => 'Article added successfully'), 'result' => array('data' => $this->obj));
				}else{
					// $this->session->set_flashdata('error', 'Unable to adde Product');
					// redirect('admin/product/add', 'refresh');
					$this->response = array('status' => array('error_code' => 1, 'message' => 'Unable to add Article'), 'result' => array('data' => $this->obj));
				}
			}else{
			    $this->data['updated_at'] = date('Y-m-d');
				if($this->common_model->update('articles', $this->data,['article_id'=> $postData['article_id']])){
					/**
					 * Add multiple images 
					*/
					$this->data = [];
				...
				// 		if(!empty($this->data)){
				// 			//remove old images
				// 			$this->db->where(['article_id'=> $postData['id']]);
				...
				// 	}
					...
					$this->response = array('status' => array('error_code' => 0, 'message' => 'Article saved successfully'), 'result' => array('data' => $this->obj));
				}else{
					// $this->session->set_flashdata('error', 'Unable to update product');
					// redirect('admin/product/edit/'.$postData['product_id'], 'refresh');
					$this->response = array('status' => array('error_code' => 1, 'message' => 'Unable to save Article'), 'result' => array('data' => $this->obj));
				}
			}
		}
		else {
	...
	public function get()
	{
		...
		if (!empty($postData)) {
			...
			if($postData['source'] === 'WEB'){
				$where = array('articles.status !='=> 3);
			}else{
				$where = array('articles.status'=> 1);
			}
			$select = 'articles.*';
			$this->obj = $this->common_model->select('articles', $where, $select, 'articles.article_id ', 'DESC');
			//echo $this->db->last_query(); die;

			//if($postData['source'] === 'WEB'){
				$this->data['articles'] = $this->obj;
				$html = $this->load->view('admin/ajax-view', $this->data, true);
				$this->response = array('status' => array('error_code' => 0, 'message' => 'Success'), 'result' => $html, 'data'=> $this->data['articles']);
			// }else{
			// 	if(!empty($this->obj)){
