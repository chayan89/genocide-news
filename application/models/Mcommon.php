...
//echo $this->db->last_query();exit;
    ...
    */
    ...
    ///for msg list
    public function getMessageList($lead_id){
        $sql ="SELECT lead_message.*,users.name FROM  lead_message left join users on lead_message.user_id = users.id where lead_message.lead_id=".$lead_id." order by id DESC";
        $Query = $this->db->query($sql);     
        return $Query->result_array();           
    }

    public function vendorlist($table,$condition,$join,$joinType)
    { 
        $this->db->select('vendor_details.vendor_name AS vendor_name,IF(vendor_details.vendor_image IS NULL, "", CONCAT("'.site_url().'uploads/vendor/",vendor_details.vendor_image)) AS vendor_image, offer.*');
        $this->db->from($table);
        if (isset($join)) {
    ...
    public function orderlist($table,$condition,$join,$joinType)
    { 
        $this->db->select('users.user_id AS user_id, users.fname AS fname, users.mobile AS mobile,orders.*');
        $this->db->from($table);
        if (isset($join)) {
            foreach ($join as $key => $value) {
                $this->db->join($key, $value,$joinType);
    ...
    public function membership_list($table,$condition,$join,$joinType)
    { 
        $this->db->select('membership_types.membership_type AS membership_type,subscription.*');
        $this->db->from($table);
        if (isset($join)) {
            foreach ($join as $key => $value) {
                $this->db->join($key, $value,$joinType);
        ...
        $this->db->order_by('subscription.subscription_id', 'desc');
        $query=$this->db->get();
        //echo $this->db->last_query();exit;
        return $query->result_array();
    ...
    public function deshboardorderlist($table,$condition,$join,$joinType)
    { 
        $this->db->select('users.user_id AS user_id, users.fname AS fname, users.mobile AS mobile,orders.*');
        $this->db->from($table);
        if (isset($join)) {
    ...
    public function getRows($table,$condition ,$order_by=''){
